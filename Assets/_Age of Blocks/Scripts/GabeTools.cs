...
public static class GabeTools
{
	...
	//
	public static Quaternion RotateTowards2D(Vector2 self, Vector2 target)
	{
	...
	// Generates sine wave points.
	public static List<Vector3> GenerateSineWavePoints(Vector3 fromPos, Vector3 toPos)
	{
		List<Vector3> _listSineWavePoints = new List<Vector3>();
		float waveHeight = 1f;
		float numberOfPoints = 15;
		float halfWaveHeight = waveHeight * 0.5f;
		float length = Vector3.Distance(fromPos, toPos);
		float step = length / numberOfPoints;
		float freq = 1;

		for (float i = 0; i < numberOfPoints; ++i)
		{
			//_listSineWavePoints.Add(new Vector3(stepX, Mathf.Sin(i * step) * halfWaveHeight, 0));
			float rand = UnityEngine.Random.Range(-0.1f, 0.1f);
			_listSineWavePoints.Add(new Vector3(Mathf.Abs(i * step), Mathf.Sin(i * step * freq) * halfWaveHeight * rand, 0f));
		}

		return _listSineWavePoints;
	}
	...
	// PLAYER PREFS X
	//
	// Nope, I didn't implement the stuff below. I took it from http://wiki.unity3d.com/index.php/ArrayPrefs2#C.23_-_PlayerPrefsX.cs */
	//------------------------------------------------------------------------------------------------------------------------------------
